...
The server application is basically a simple EJB application exposing a messaging destination and publishing stock price
...
----
<1> The +ServerFilter+ annotation triggers GraniteDS initialization with an EJB configuration. Note the use of the
property +useBigDecimal+ to tell GDS that it should serialize +BigDecimal+ as is and not convert them to doubles so
there is no loss of precision during the transfer.
...
The publisher is a simple singleton EJB. It gets a reference to the main +Gravity+ publishing component with +@EJB+:
[source,java,indent=0]
